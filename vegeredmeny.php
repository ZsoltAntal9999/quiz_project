...
<?php
    ...
    extract($_POST);
    $sql="Select jatekosnev, rossz from rekord where kontinens_id={$kontinensidj}";
    $stmt=$db->query($sql);
    if($stmt->rowCount()>0){
        $row=$stmt->fetch();
        extract($row);
        ...
        $szoveg="Gratulálunk! Itt te vagy az első játékos.";
        $str="<table class='table table-bordered'>
            <thead class='thead-dark'>
                <th>Kontinens</th>
                <th>Legjobb játékos neve</th>
            ...
            <tbody>
                <tr class='table-success'>
                <td id='rekord_kontinensid'>{$kontinensj}</td>
                <td id='rekord_jatekos'>&nbsp;</td>
                <td id='rekord_rossz'>&nbsp</td>
                </tr>
            </tbody>
            <caption>
                A te rossz találataid száma: {$rosszj} db <br> {$szoveg}
